    <!--header start-->
    <?php include 'require/header1.php'; ?>
    <!-- Inner page hedaing start -->

    <section class="irs-inner-page-heading irs-layer-black">
        <div class="container">
            <div class="row">
                <div class="col-md-12"><br><br>
                   <div class="irs-inner-heading">
                        <h2>TRANSPORT</h2>
                        <i class="icofont icofont-education"></i>
                        <p><a href="index.php">HOME</a> > <a href="transport.php">TRANSPORT</a>
                        </p>
                    </div> 
                </div>
            </div>
        </div>
    </section>
    <!-- Inner page hedaing end -->

    <section class="irs-gallery-field">
        <div class="container">
            <div class="row">
                <!-- <div class="col-md-6 col-md-offset-3"> -->
                    <div class="col-md-12">
                    <div class="irs-section-title">
                        <h2 style="font-size:24px;"><span>TRANSPORT DETAILS</span></h2>
                        <div class="irs-title-line">
                            <div class="irs-title-icon">
                                <i class="icofont icofont-education"></i>
                            </div>
                        </div>
    
                    </div>
                  
                </div>
            </div>
        </div></section>

        <section class="Service-content">
            <div class="detail-content">            <h2>About Transport Facilities</h2>
            <p>
            GMS MAVMM has more than 10 Buses which are made available to the students and the faculty members for commuting from various points in the city and also from the suburbs like AnnaNagar, Therkuvasal, Periyar, Kalavasal, Checkanurani, Keelavasal, and Theppakulam to the campus and back.
            </p>
            </div>
        </section>
        <style type="text/css">
       .Service-content{
        width: 100%;
        display: flex;
        justify-content: center;
        align-items: center;
        flex-direction: column;
        margin: 10px 0px 40px;
       }
       .detail-content{
        width: 400px;
        height: fit-content;
        text-align: center;
       }
       @media (max-width: 500px){
        .detail-content{
        width: 80vw;
        height: fit-content;
        text-align: center;
       }
    }
    </style>

 
        <div class="irs-courses-col irs-blog-col">
        <img src="image/clg/trans1.jpg" align="center" style="width:100%;">
   
</div>
    
     <section class="irs-blog-field">
        <div class="container">
            <div class="row animatedParent animateOnce">

                <div class="col-md-4 col-sm-6">
                    <div class="irs-courses-col animated fadeInLeftShort slow delay-250">
                        <div class="irs-courses-img">
                            <img src="image/clg/trans2.jpg" style="height:250px;" alt="">
                            
                            
                        </div>
                        <div class="irs-info-text">
                            <ul class="clearfix">
                                <li>
                                    <p><i class="icofont icofont-clock-time"></i> <a href="#">14 Sep 2017</a>
                                    </p>
                                </li>
                                <li>
                                    <p><i class="icofont icofont-open-eye"></i> <a href="#">65</a>
                                    </p>
                                </li>
                                <li>
                                    <p><i class="icofont icofont-comment"></i> <a href="#">40</a>
                                    </p>
                                </li>
                            </ul>
                        </div>
                        <div class="irs-courses-content">
                          <p>TRANSPORT</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 col-sm-6">
                    <div class="irs-courses-col animated fadeInUpShort slow delay-500">
                        <div class="irs-courses-img">
                            <img src="image/clg/trans3.jpg" style="height:250px;"alt="">
                            
                        </div>
                        <div class="irs-info-text">
                            <ul class="clearfix">
                                <li>
                                    <p><i class="icofont icofont-clock-time"></i> <a href="#">14 Sep 2017</a>
                                    </p>
                                </li>
                                <li>
                                    <p><i class="icofont icofont-open-eye"></i> <a href="#">65</a>
                                    </p>
                                </li>
                                <li>
                                    <p><i class="icofont icofont-comment"></i> <a href="#">40</a>
                                    </p>
                                </li>
                            </ul>
                        </div>
                        <div class="irs-courses-content">
                            <p style="text-align: center;">TRANSPORT</p>
                        </div>
                    </div>
                </div>

            </div></div></section>
              

    <!--------------footer start----------------->
    <?php include 'require/footer.php'; ?>
    <!-- Footer end -->

   <!--  <section class="irs-copyright-field">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    <div class="irs-copyright">
                        <p>Copyright (c)2015 GMS MAVMM Polytechnic College. Powered by <a href="#">GMS MAVMM</a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </section> -->
